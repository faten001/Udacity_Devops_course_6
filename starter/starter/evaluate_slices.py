...
# I need to load the model, reload the encoders and 
# can I do this on the whole data or only the test data?


def test_feature_slice(feature):
    # load the model and encoders
    results_df = pd.DataFrame(columns=['slice','precision','recall','fbeta','size'])
    ...
    for value in data[feature].unique():
        ...
        results_df.loc[len(results_df)] =  [value,precision,recall,fbeta,len(data_slice)]
    print(results_df)
...
def prepare_data(data,encoder,lb):
    ...
    X, y, _, _ = process_data(data,categorical_col,'salary',False,encoder,lb)
    return X, y
