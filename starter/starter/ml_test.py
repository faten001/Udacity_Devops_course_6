...
# make sure the function return none when in test mode
# and the encoder where not passed.
...
def test_target_removed(data, categorical_columns):
    X, y, encoder, lb = process_data(data, categorical_columns, training=True,
                                     label='salary')
    X = np.transpose(X)
